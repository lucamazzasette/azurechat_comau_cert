...
class ChatState {
  ...
  private async chat(formData: FormData) {
    // Don't trigger auto-scroll for user messages
    this.updateAutoScroll(false);
    this.loading = "loading";
        ...
        if (event.type === "event") {
          ...
          switch (responseType.type) {
            ...
            case "content":
              ...
              // Enable auto-scroll for assistant messages during streaming
              this.updateAutoScroll(true);
              
              // While streaming, ensure the "loading" state is still active
              // This allows proper scroll behavior during message streaming
              this.loading = "loading";
  ...
  public async submitChat(e: FormEvent<HTMLFormElement>) {
    ...
    if (this.chatThreadId === "" || this.chatThreadId === undefined || isOnHomePage) {
      ...
      // Dispatch a custom event to notify that a new conversation is starting
      // This allows the useChatScrollAnchor hook to reset its first message interaction state
